...
public class KernelHelper {
    ...
    public static int calculateKernelSize(final byte[] kernel) throws RuntimeException {
        ...
        int kernel_size = Integer.reverseBytes((kernel[8] << 24) + (kernel[9] << 16) + (kernel[10] << 8) + (kernel[11] << 0));
        int ramdisk_size = Integer.reverseBytes((kernel[16] << 24) + (kernel[17] << 16) + (kernel[18] << 8) + (kernel[19] << 0));
        int second_size = Integer.reverseBytes((kernel[24] << 24) + (kernel[25] << 16) + (kernel[26] << 8) + (kernel[27] << 0));
        int page_size = Integer.reverseBytes((kernel[36] << 24) + (kernel[37] << 16) + (kernel[38] << 8) + (kernel[39] << 0));
        int dt_size = Integer.reverseBytes((kernel[40] << 24) + (kernel[41] << 16) + (kernel[42] << 8) + (kernel[43] << 0));
        int pages = 1;
